...
**2026-07-03 (later 2): class field — required + self-building dropdown.** Driver: school gave no class roster; students invent spellings ("grade 8b" vs "Grade 8B") which splits the roster into duplicate classes. Deployed LIVE: `booking-availability` v7 returns `event.classNames` (distinct class names for the school from students + non-cancelled bookings, case-insensitive dedupe, roster spelling wins, capped 100 — no student names exposed); `booking-create` v14 REQUIRES class server-side (400 with friendly message) and canonicalizes the typed class case-insensitively against known names via `canonicalClassName()` (whitespace collapsed). Web repo: `app/book/route.ts` class input got `list='clslist'` + `<datalist>` populated in `renderHead()` via DOM (no escaping issues) — students see existing classes as suggestions while typing. Page already client-required the field; server now enforces.

**2026-07-05 (later): studio LOGO on the booking page.** `photographers.logo_url` held junk ("www.whitephoto.ca"); his real logo.png (158KB, Mar 23) was already in the public `studio-logos` bucket under `<user_id>/logo.png` — logo_url now points at it (SQL). `booking-availability` v10 returns `studioLogoUrl` (only when it matches ^https:// — guards against junk values). `app/book/route.ts` (uncommitted): `#slogo` div above the title renders it via DOM (max-h 56px, onerror hides). Flutter: `syncStudioProfileToCloud` gained `logoLocalPath` — on profile Save it uploads png/jpg/jpeg/webp (HEIC skipped) to `studio-logos/<auth uid>/logo.<ext>` (upsert; storage RLS folder = auth.uid) and sets `logo_url` with a `?v=` cache-buster; both profile save handlers pass `result.logoPath`. Contact-fields sync and logo sync are separate try blocks so one failing never blocks the other.

**2026-07-05 (later): studio LOGO on the booking page + PNG logo picker fix.** `photographers.logo_url` held junk ("www.whitephoto.ca"); his real logo.png (158KB, Mar 23) was already in the public `studio-logos` bucket under `<user_id>/logo.png` — logo_url now points at it (SQL). `booking-availability` v10 returns `studioLogoUrl` (only when it matches ^https:// — guards junk values). `app/book/route.ts` (uncommitted): `#slogo` div above the title renders it via DOM (max-h 56px, onerror hides). Flutter: `syncStudioProfileToCloud` gained `logoLocalPath` — on profile Save it uploads png/jpg/jpeg/webp (HEIC skipped) to `studio-logos/<auth uid>/logo.<ext>` (upsert; storage RLS folder = auth.uid) and sets `logo_url` with `?v=` cache-buster; both profile save handlers pass `result.logoPath`; contact sync and logo sync are separate try blocks. ALSO: logo file pickers were greyed-out for PNG (they reused `kSourcePhotoPickerExtensions`, which intentionally excludes PNG) — new `kLogoPickerExtensions` (png/jpg/jpeg/webp) in photo_utils.dart, swapped into all THREE logo pickers (order_forms_screen ~659 + ~2569, project_sorter_screen ~6840); the real photo-import picker untouched. ALSO (post-rebuild feedback "logo goes away when I exit"): picking a logo in either Studio Profile sheet now persists IMMEDIATELY (saveGlobal(logoPath) + cloud sync inside `_pickLogo`) instead of waiting for the top-right Save; both sheets' initState got `_restoreSavedLogo()` fallback (loads global logoPath + bytes when parent passed none) so the saved logo always shows on reopen. Needs ANOTHER `flutter build macos`.

**2026-07-05 (later 2): expiration date on school + event list cards.** Driver: Harout wants gallery expiry visible on the card thumbnails. `expiration_date` is the live column (settings pages edit it; `archive-expired-galleries` cron reads projects.expiration_date; schools.gallery_expires_at is legacy/unused). Changes: `app/api/dashboard/events/route.ts` select += expiration_date; `app/dashboard/projects/events/page.tsx` + `app/dashboard/schools/page.tsx` — `expiryInfo()` helper renders "Expires Mar 3, 2027" under the shoot-date line (gray normally, amber ≤14 days out, red "Expired ..." once past; hidden when unset). Currently 1/9 schools + 5/19 events have an expiry set. Typecheck clean.

**2026-07-05: booking page contact line + punctuality note + email reply-to.** Deployed LIVE: `booking-availability` v9 adds `studioName/studioPhone/studioEmail` (from photographers) + `slotMinutes` to the event payload; `booking-send-emails` v10 sets Resend `reply_to` = studio email on all CLIENT emails (replies used to go to noreply@). Web repo `app/book/route.ts` (uncommitted): amber notebox under the fee ("arrive a few minutes early — each session is just N minutes…", N = slot_duration_minutes) + "Questions? Contact WHITE PHOTO — phone · email" line with tel:/mailto: links (renders only fields that exist). ⚠️ `photographers.studio_phone` is EMPTY for Harout — fill it in settings (or SQL) and the phone appears automatically. Also this session: renamed cloud school "TEST"→"triOS College Business Technology Healthcare" (463e6864, Aug 18 event 9a3217b9), deleted the old un-deletable "Test" school + its Jul 15 event chain + 28 empty Jun-17 demo schools (kept everything with students/orders). Fee now $30; **sitting_fee_description still empty — paste the non-refundable/credit policy text before sending the link.**
...
**Flutter (rebuild required) — 2026-07-05 addition: Studio Profile now syncs to cloud.** Driver: the Order Forms "Studio Profile" (email/phone/address) saved ONLY to local OrderFormStore — cloud `photographers.studio_*` had stale hotmail + empty phone while the profile showed Harout@whitephoto.com / 647-273-8052 (cloud manually aligned via SQL this session). Fix: new `SupabaseService.syncStudioProfileToCloud({email,phone,address})` (updates photographers by `user_id`, best-effort/never throws; RLS "Photographer updates own record" allows it) called from BOTH Studio Profile save handlers (`order_forms_screen.dart` ~line 2640 + `project_sorter_screen.dart` ~line 6960). From now on: edit profile in app → Save → booking page contact line + email reply-to update automatically.
...
Folder tab also shipped this session — pick any folder → recursive scan via `isSupportedSourcePhoto` (JPG/JPEG/RAW/HEIC/etc.) → preview list → pick destination school + class → File.copy each photo to `<captureBase>/<className>/`.  Skips duplicates by filename so re-runs are safe.  Cancel button mid-import.  Final summary dialog.

**Folder tab v2 (later same session)** — major upgrade adding thumbnails + per-photo selection + duplicate detection + sort + remember-last-destination + tap-for-preview lightbox.  New `PhotoCandidate` model w/ `autoStars` + `autoColor` hooks; new `_PhotoTile` widget renders `Image.file(cacheWidth:240)` with a checkbox overlay, "Already imported" green badge, optional star + color-dot rendering, and a magnifier button that opens `_PhotoLightbox` (full-screen prev/next).  Sort dropdown: newest / oldest / name.  Last destination saved via `LocalStore.readAppPrefs() / writeAppPrefs()` with keys `import_hub_last_school_id` + `import_hub_last_class_name`.  After scan, `_refreshDuplicateFlags` re-checks every candidate against the destination so changing the school/class updates the green badges live.

**Assisted Culling (Lightroom-style, also same session)** — fully real local-AI signal, no fakery:
- New service `lib/services/assisted_culling_service.dart` — `analyzePhoto(File)` returns `CullingScores { subjectSharpness, exposureStatus, likelyMisfire, captureTime, combinedScore, autoStars, autoColor }`.  Uses pure-Dart Laplacian variance for sharpness, histogram clipping for exposure, avg-luma threshold for misfire detection, EXIF DateTimeOriginal for capture time.  Downsamples to 400px wide for ~50ms/photo perf, yields to event loop every 5 photos so UI stays responsive on a 2,000-photo run.
- Mounts as a 280px left sidebar (`AssistedCullingPanel`) inside the Folder tab — mirrors the Lightroom Classic Assisted Culling layout exactly (Select / Reject / Auto-Stack / View sections, Soft↔Sharp slider for Subject Focus, Re-analyze button, live "Analyzing N of M" progress strip).
- Auto-runs after every folder scan; scores stream in and update tile state live.  Auto-selection: green/yellow stay ticked, red flips off.  Photographer can override either way at any time.
- View toggle: Selects / Rejects / All — filters the grid to just the auto-keepers, just the auto-rejects, or everything.  Each pill shows a live count.
- Per-tile rendering: small color dot (top-left) + star count badge (bottom-left) when scores have landed.  Photos still being analyzed show no badges.
- Honestly-marked "Studio AI v2" placeholders for Eye Focus / Eyes Open / Documents / Visual Similarity — these need a TFLite face-detection model bundle (MediaPipe BlazeFace or similar) which is a multi-day integration on macOS desktop, not session work.  Listed in the panel UI as disabled rows with a "Needs Studio AI v2" caption so it's clear what's coming, not silently broken.
- `google_mlkit_face_detection` plugin IS in pubspec but it's iOS/Android only as of v0.13.2 — does not run on macOS desktop.  TFLite + custom model bundle is the path forward for Studio AI v2.

Followups for next session (already scoped in design doc):
1. **SD Card tab:** auto-detect plugged-in volumes (`/Volumes/` on macOS, drive letter scan on Windows, `/media/<user>/` on Linux), show a card per detected card with photo count + size, "Import to school..." action that copies into the chosen school+class folder.  Most of the per-photo copy plumbing is already done (see `_runImport` in the Folder tab) — SD tab can lift that and just add volume detection on top.
2. **nobg PNG download:** today the school photo download brings originals only — composites built on a fresh computer still need a re-run of background removal.  Optional polish: when pulling a school, also fetch any `nobg-photos/...` keys associated with the student photo paths.
3. **Conflict merge mode:** today the 3-button prompt offers Overwrite / Skip / Cancel.  A 4th option ("Merge: cloud wins on new rows, keep local rows that exist") would preserve unpushed local edits while still pulling new students added on the other computer.

**Cover photo polish (web + Flutter, same session)** — three places fixed:
1. **Web `/dashboard/schools` cards** (`app/dashboard/schools/page.tsx` line ~571): switched from `objectFit:cover` (which chops heads off portrait shots) to a layered render — blurred copy of the cover photo as the backdrop fill (28px blur, 0.55 brightness, scale 1.15) with the actual photo on top via `objectFit:contain`.  Apple TV / Netflix pattern.  Heads no longer cropped, gradient sides feel intentional.  Saved focal point still positions the contained image.
2. **Web `/dashboard/projects/events` cards** (`app/dashboard/projects/events/page.tsx` line ~430): same pattern applied for event project cards.
3. **Flutter Import Hub Cloud tab**: `fetchCloudInventory` now selects `cover_photo_url` from both `schools` and `projects`, threads it into each inventory row.  New `_InventoryThumbnail` widget renders `Image.network(coverUrl, cacheWidth: 88, fit: BoxFit.cover)` with a graceful icon fallback when the URL is empty or load fails.  Replaces the placeholder colored-square + icon — visual ID at a glance for the photographer browsing 47 schools.

Followups for the cover-photo polish:
- The bottom-tab Cloud screen (`lib/screens/cloud_screen.dart`) still shows dark gradient cards without cover thumbs.  Same `_InventoryThumbnail` pattern can lift in for that next session.
- The `/m/schools` and `/m/events` mobile pages have their own `objectFit:cover` blocks (lines 355, 429, 711, 382, 398) — these are usually wider hero banners so cropping is less severe, but worth a polish pass eventually.

**Phase A — Destination picker (School / Event + inline create)** shipped on the Folder tab this session.  New widget at `lib/widgets/destination_picker.dart`:

- Top-level toggle: **School** vs **Event** (two pill buttons, dark-fill on active).
- School path: school dropdown + **+ New** button (opens inline name field, saves via `LocalStore.addSchool`) → class dropdown populated from existing students' `class_name` distinct values + **+ New** button (inline text field, sets `subFolder`).
- Event path: project dropdown filtered to `workflowType=project` + **+ New** (inline name field, saves via `LocalStore.addProject`) → album dropdown populated from `LocalStore.loadProjectAlbums(projectId)` + **+ New** (creates via `LocalStore.createProjectAlbum`, idempotent).
- Exposes a single `ImportDestination { mode, localSchoolId, subFolder }` struct that consumers use to resolve `<captureBase>/<subFolder>/`.

Wired into the Folder tab — replaces the old school-dropdown + free-text class field.  Last destination persisted via `LocalStore.readAppPrefs / writeAppPrefs` with new `import_hub_last_mode` key alongside the existing `import_hub_last_school_id` / `import_hub_last_class_name`.

Wired into the SD Card tab — replaces the old school-dropdown + free-text class field.  Last destination persisted via `LocalStore.readAppPrefs / writeAppPrefs` with new `import_hub_last_mode` key alongside the existing `import_hub_last_school_id` / `import_hub_last_class_name`.
